	...
	def verify_inside_period
 	...
 	def not_overlap_phases
 		...
 		if !self.evaluation_period.nil?
 			...
 			if self.evaluation_period.persisted?
			  ...
			  when PLANNING
			  	if self.end_date_phase <= phases.second.start_date_phase
						...
						errors.add(:end_date_phase, "O período da fase de Planejamento não deve sobrepor o período da fase de Acompanhamento.")
					end
				...
				when MONITORING
				 	if( self.start_date_phase >= phases.first.end_date_phase and
						...
						# Nothing To Do
				 		...
				 		errors.add(:end_date_phase, "O período da fase de Acompanhamento não deve sobrepor o período da fase de Planejamento ou de Formalização.")
					end
				...
				when FORMALIZATION
				 	if( self.start_date_phase >= phases.second.end_date_phase and
						...
						# Nothing To Do
				 		...
				 		errors.add(:end_date_phase, "O período da fase de Formalização não deve sobrepor o período da fase de Acompanhamento ou de Plano de Desenvolvimento.")
				 	end
				...
				when DEVELOPMENT_PLAN
				 	if( self.start_date_phase >= phases.third.end_date_phase )
						...
						errors.add(:end_date_phase, "O período da fase de Plano de Desenvolvimento não deve sobrepor o período da fase de Formalização.")
					end
				else
				 	#Nothing to do
				end
